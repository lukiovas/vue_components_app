<template>
  ...
  <input type="text" v-model="property_id" @keyup="searchProperties(property_id)" class="form-control" placeholder="Search" aria-label="Search" aria-describedby="basic-addon2">
                ...
                <thead>
                    ...
                    <th></th>
                        ...
                        <tbody v-for="property in properties" v-bind:key="property.id">
                                ...
                                <td  scope="col"><button @click="editProperty(property); show();" class="btn btn-warning">Edit</button></td>
                                <td  scope="col"><button @click="deleteProperty(property.id)" class="btn btn-danger">Delete</button></td>
                                </tr>
                        </tbody>
    ...
    methods: {
      ...
      this.value.push(tag)
        ...
        searchProperties(property_id) {
             axios.get('http://localhost:8080/api/properties/search='+property_id).then((response) => {
                this.properties = response.data;
            })
        },
        showProperty() {
        ...
        createProperty(property) {
                    if (this.edit===false) {
                        axios.post('http://localhost:8080/api/properties/',  
                        this.property).then(data => { 
                            this.property=this.emptyProperty;
                        this.fetchProperties();
                    })

                    } else{
                        axios.put('http://localhost:8080/api/properties/'+this.property.id,  
                        this.property).then(data => {
                            this.property=this.emptyProperty;
                        this.fetchProperties();
                                })
        ...
        deleteProperty(id) {
            if(confirm('Are you sure?')) {
                axios.delete('http://localhost:8080/api/properties/'+id, { data: {id: 'id'}}).then( data => {this.fetchProperties()} )
            }
        }
    }
}
