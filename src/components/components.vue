<template>
  ...
  <input type="text" v-model="component_id" @keyup="searchComponents(component_id)" class="form-control" placeholder="Search" aria-label="Search" aria-describedby="basic-addon2">
    ...
    methods: {
      ...
      this.value.push(tag)
        ...
        searchComponents(component_id) {
             axios.get('http://localhost:8080/api/components/search='+component_id).then((response) => {
                this.components = response.data;
            })
        },
        showComponent() {
        ...
        createComponent(component) {
                    if (this.edit===false) {
                        axios.post('http://localhost:8080/api/components/',  
                        ...
                        axios.put('http://localhost:8080/api/components/'+this.component.id,  
                        this.component).then(data => {
                            this.component=this.emptyComponent;
                        this.fetchComponents();
                                })
        ...
        deleteComponent(id) {
            if(confirm('Are you sure?')) {
                axios.delete('http://localhost:8080/api/components/'+id, { data: {id: 'id'}}).then( data => {this.fetchComponents()} )
            }
        },
