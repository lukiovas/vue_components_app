      ...
      <div class="container">
        <div class="float-left">
          <ul class="nav nav-pills">
            ...
            <li class="nav-item">
              <a
                href="http://localhost:8081/components/#/Templates"
  ...
  <div class="input-group-append">
    ...
    <modal name="create-template" :adaptive="true" height="auto" width="800" :scrollable="true">
      <form
        ...
        action="http://localhost:8081/templates"
        method="post"
      >
        <div class="line">
    ...
    <modal name="show-template" :adaptive="true" height="auto" width="800" :scrollable="true">
      <table class="table template-table">
        <tbody>
          <tr>
            <td>Template Name</td>
            <td>{{this.template.name}}</td>
          </tr>
          <tr>
            <td>Text</td>
            <td>
              <pre>{{this.template.text}}</pre>
            </td>
          </tr>
        </tbody>
      </table>
    ...
    <table class="table">
      ...
      <tbody v-for="template in templates" v-bind:key="template.id">
        <tr class="click">
          <td @click="editTemplate(template); showTemplate();" scope="col">
            <b>{{template.name}}</b>
          </td>
          <td
            v-if="template.text.length>35"
            class="text"
            @click="editTemplate(template); showTemplate();"
            scope="col"
          >{{template.text.substring(0,34)+"..."}}</td>
          <td
            v-else
            class="text"
            @click="editTemplate(template); showTemplate();"
          ...
          >{{template.text}}</td>
          <td scope="col">
            <button @click="editTemplate(template); show();" class="btn btn-warning">Edit</button>
          </td>
          <td scope="col">
            <button @click="deleteTemplate(template.id)" class="btn btn-danger">Delete</button>
          </td>
        </tr>
      </tbody>
    </table>
...
export default {
  ...
  methods: {
    ...
    createTemplate(template) {
      if (this.edit === false) {
        axios
          .post("http://localhost:8080/api/Templates", this.template)
          .then(data => {
        ...
        axios
          .put(
            "http://localhost:8080/api/Templates/" + this.template.id,
            this.template
          )
          .then(data => {
            this.template = this.emptyTemplate;
    ...
    deleteTemplate(id) {
      if (confirm("Are you sure?")) {
        axios
          .delete("http://localhost:8080/api/Templates/" + id, {
            data: { id: "id" }
          })
          .then(data => {
            this.fetchTemplates();
